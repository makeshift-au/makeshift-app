...
export default function ArtistCard({ artist }: { artist: Artist }) {
  return (
    ...
    >
      {/* Image placeholder */}
      <div className={`${artist.bg} h-52 relative`}>
        {artist.featured && (
          <span className="absolute top-3 left-3 bg-lime text-black font-mono text-[10px] font-bold tracking-[0.1em] px-3 py-1 rounded-full uppercase">
            Featured
          </span>
        )}
        {artist.commissions && (
          <span className="absolute top-3 right-3 bg-blue/80 text-white font-mono text-[10px] font-bold tracking-[0.1em] px-3 py-1 rounded-full uppercase">
            Commissions
          </span>
        )}
      </div>
