...
function statusBadge(a: any) {
  ...
  return (
    <span
      className={`inline-block font-mono text-[10px] font-bold tracking-[0.1em] px-3 py-1 rounded-full uppercase ${cls}`}
    >
...
export default async function AdminArtistsPage() {
  ...
  const onboardingCount = allArtists.filter(
    (a: any) => a.status === "onboarding",
  ).length;
            ...
            {allArtists.map((a: any) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 rounded-lg flex-shrink-0 ${a.bg ?? "bg-acid"}`}
                  />
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-midgrey mb-3">
                  <span className="capitalize">
                    {a.discipline?.replace("-", " ")}
                  </span>
                  <span>{a.location}</span>
                  <span className="font-mono">{a.fee_rate}% fee</span>
                </div>
                <ArtistRosterActions
                  artistId={a.id}
                  slug={a.slug}
